...
$GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['t3lib/class.t3lib_tcemain.php']['clearCachePostProc'][] = \NITSAN\NsCloudflare\Hooks\TCEmain::class . '->clear_cacheCmd';
...
if (filter_var($config['enablePurgeByTags'] ?? false, FILTER_VALIDATE_BOOL)) {
    $GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['tslib/class.tslib_fe.php']['insertPageIncache'][$_EXTKEY] =
        \NITSAN\NsCloudflare\Hooks\ContentProcessor::class;
    $GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['tslib/class.tslib_fe.php']['contentPostProc-output'][$_EXTKEY] =
        \NITSAN\NsCloudflare\Hooks\ContentProcessor::class . '->sendPageCacheTag';
}
if (!empty($config['apiKey'])) {
    $GLOBALS['TYPO3_CONF_VARS']['BE']['toolbarItems'][] = \NITSAN\NsCloudflare\Backend\ToolbarItems\CloudflareToolbarItem::class;
}
