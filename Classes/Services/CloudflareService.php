...
namespace NITSAN\NsCloudflare\Services;
...
class CloudflareService implements SingletonInterface
{
    ...
    public function send($route, array $parameters = [], string $request = 'GET')
    {
        if (!trim($this->config['apiKey'] ?? '')) {
            throw new \RuntimeException('Cannot invoke data send for Cloudflare: Invalid apiKey for EXT:ns_cloudflare', 1337770232);
        }

        if (!$this->config['useBearerAuthentication'] && !GeneralUtility::validEmail(trim($this->config['email']))) {
            throw new \RuntimeException('Cannot invoke data send for Cloudflare: Invalid email for EXT:ns_cloudflare', 1337770383);
        }
        ...
        if ($request === 'GET') {
            ...
            if (isset($data['success']) && isset($data['result_info']['total_pages']) > 1) {
